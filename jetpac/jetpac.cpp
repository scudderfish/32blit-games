...
int hitlaser(Point pos)
{
  Vec3 laser;
  if (dir == LEFT)
    laser = Vec3(player.x - 200, player.y, player.x);
  if (dir == RIGHT)
    laser = Vec3(player.x, player.y, player.x + 200);
  if ((pos.x > laser.x) && (pos.x < laser.z) && abs(laser.y - pos.y) < 2)
    return 1;
  return 0;
}
...
void gameloop()
{
  ...
  if (player.x > screen.bounds.w)
    player.x = 0;
  if (player.x < 0)
    player.x = screen.bounds.w;
  ...
  if (takeoff == -10)
  {
    ...
    if (dir == RIGHT)
      screen.sprite(costume, player, SpriteTransform::HORIZONTAL);
    if (fire == 1)
      laser();
  }
  ...
  {
    colorsprites(white);
    if ((i == rocketsmade) && collide(rocketpos[i], player))
      rocketgrabbed[i] = 1;
    if (rocketgrabbed[i])
    {
    ...
    if (!hitplatform(rocketpos[i] + Point(0, 10)))
    {
      ...
      if (dropzone(rocketpos[i].x) && (rocketpos[i].y > rocketpos[i - 1].y - 16))
      {
        rocketsmade++;
      }
    }
  ...
  if (gem == 0)
    colorsprites(colors[RND(6)]); // sparkle diamond
  if (gem == 2)
    colorsprites(colors[RND(2)]); // flash isotope
  screen.sprite(gems[gem], gempos);
  if (!hitplatform(gempos))
    gempos.y++;
  ...
  for (int n = 0; n < NUMALIENS; n++)
  {
    ...
    if (pos.x < 0 || pos.x > screen.bounds.w)
    {
      newalien(n);
    }
    ...
    if (aliendir[n].x < 0)
      screen.sprite(costume, pos, SpriteTransform::HORIZONTAL);
    else
      screen.sprite(costume, pos);
  }
  // player
  if (playerdied)
  {
